...
const HomePage = () => {
  ...
  let handleClickStatistic = async() => {
    let datas = await axios.post(
      "http://localhost:8080/orders/orders-by-date-between",
      {
      ...
      <div className="statistic-orders">
        <div className="statistic-orders-by-year">
          <div className="select-dates">
            <select value={employeeChosen} className="select-employee" onChange={(e) => setEmployeeChosen(e.target.value)}>
              <option value={0}>All</option>
              {
                employees.map(e => (
                  <option key={e.empId} value={e.empId}>{e.fullName}</option>
                ))
              }
            </select>
            From : <input type="date" value={fromDate} className="date" onChange={(e) => setFromDate(e.target.value)}/>
            To : <input type="date" value={toDate} className="date" onChange={(e) => setToDate(e.target.value)}/>
            <button className="statistic" onClick={handleClickStatistic}>Thống Kê</button>
          </div>
          <Line
            options={optionsLine}
            data={dataLine}
